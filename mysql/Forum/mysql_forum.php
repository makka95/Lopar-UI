...
function display_categories () {
	...
	while ($query_result = mysqli_fetch_assoc($query_run)) {
		...
		echo '<div class="categorie" onclick="window.location.assign(\'category.php?id=' . $query_result['ID'] .'\')"><a id="cat" href="category.php?id=' . $query_result['ID'] .'">';
...
function display_threads() {
	...
	if ($query_run) {
		while ($query_result = mysqli_fetch_assoc($query_run)) {
			...
			echo '<div class="categorie" onclick="window.location.assign(\'posts.php?thr=' . $query_result['ID'] . '\')"><a id="cat" href="posts.php?thr=' . $query_result['ID'] . '">' . $query_result['Subject'] . '</a>';
	...
	if ($query_run) {
		while ($query_result = mysqli_fetch_assoc($query_run)) {
			if ($query_result['User'] == $_COOKIE['login'] || $user_lvl == "A") {
				echo '<div class="post"><div class="postheader"><a class="edit" href="edit_post.php?id=' . $query_result['ID'] . '">Ändra</a><a class="edit" href="delete_post.php?id=' . $query_result['ID'] . '">Ta Bort</a></div><div class="post-info"><a class="creator" href="anvandare.php?nick=' . $query_result['Nick'] . '">' . $query_result['Nick'] . '</a><img alt="Profilbild" src="../User-Prof/11-prof.jpg"/></div><div class="post-text">' . $query_result['Text'] . '</div></div>';
			}
			else {
				echo '<div class="post"><div class="postheader"></div><div class="post-info"><a class="creator" href="anvandare.php?nick=' . $query_result['Nick'] . '">' . $query_result['Nick'] . '</a><img alt="Profilbild" src="../User-Prof/11-prof.jpg"/></div><div class="post-text>"' . $query_result['Text'] . '</div></div>';
			}
		}
	}
	else {
